...
def analysis(name):
    ...
    for asset in assets:
        ...
        try:
            ...
            if result["finished"]:
                ...
                print("test_nparam" in result)
                if "test_nparam" in result:
                    print(result["test_nparam"], result["test_nparam"].shape)
                print()
    ...
    rk_mean, rk_std = mean_std(rk, "rk", bprint=True)

    print()
    print("Number of Folds | Ranking Sums")
    for j, cv in enumerate(nfolds):print(cv, ": ", np.sum(rk[:, j]))
    print()
